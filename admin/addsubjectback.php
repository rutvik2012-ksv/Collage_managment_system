<?php
        ...
        {
            $sql = "SELECT *  FROM `subject` WHERE subject_name='$n1' AND subject_code='$c1' AND sem='$sem' AND  field='$dep' group by elective";
            $result = mysqli_query($conn,$sql);
            $num = mysqli_num_rows($result);
            $sql3 ="SELECT *  FROM `subject` WHERE  sem='$sem' AND  field='$dep'";
            $result3 = mysqli_query($conn,$sql3);
            $num3 = mysqli_num_rows($result3);
            $elective = "elective";
            $elective=$elective.$num3;
            echo $elective;
            if($num == 1)
            {
